...
assert(webSource.includes("SCRIPT_SRC_RE.sub(replace_script, html)"),"web não incorpora dinamicamente os scripts do HTML");
assert(webSource.includes('match.group("src")'),"web não lê os caminhos declarados no HTML");
...
assert(webSource.includes("if SCRIPT_SRC_RE.search(html)"),"Streamlit não bloqueia scripts externos não incorporados");
